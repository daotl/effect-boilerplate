...
export const StorageConfig = Config.all({
  ...
  prefix: Config.string('prefix').pipe(
    Config.nested('storage'),
    Config.orElse(() =>
      BaseConfig.pipe(
        Config.map(({ env }) =>
...
export const ApiConfig = Config.all({
  ...
  port,
  devPort: Config.integer('devPort').pipe(
    Config.orElse(() => port.pipe(Config.map(_ => _ + 1))),
  ),
...
export const MergedConfig = ApiConfig.pipe(
  Effect.andThen(apiConfig =>
    Effect.andThen(BaseConfig, baseConfig => ({ ...baseConfig, ...apiConfig })),
  ),
  Effect.cached,

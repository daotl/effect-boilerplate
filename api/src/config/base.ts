...
export const SendgridConfig = C.all({
  ...
  apiKey: C.redacted('sendgridApiKey').pipe(C.withDefault(Redacted.make(''))),
  defaultFrom: C.succeed(FROM),
  subjectPrefix: envConfig.pipe(
    C.map(env => (env === 'prod' ? '' : `[${serviceName}] [${env}] `)),
  ),
})
...
export const BaseConfig = C.all({
  ...
  sentry: C.all({
    dsn: C.redacted('dsn').pipe(
      C.nested('sentry'),
      C.withDefault(Redacted.make('???')),
    ),
  }),
  //  log: C.string("LOG").
})

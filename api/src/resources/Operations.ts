...
export const OperationsClient = Effect.gen(function* () {
  ...
  function refreshAndWaitAForOperation<R2, E2, A2>(
    ...
    return <R, E>(act: Effect<OperationId, E, R>) =>
      Effect.tap(waitForOperation(Effect.tap(act, refresh), cb), refresh)
  }
  ...
  function refreshAndWaitAForOperation_<R2, E2, A2, R, E>(
    ...
    return Effect.tap(waitForOperation(Effect.tap(act, refresh), cb), refresh)
  }

  function refreshAndWaitForOperation<R2, E2, A2>(
    refresh: Effect<A2, E2, R2>,
    cb?: (op: Operation) => void,
  ) {
    return <Req, R, E>(act: (req: Req) => Effect<OperationId, E, R>) =>
      (req: Req) =>
  ...
  function waitForOperation_(cb?: (op: Operation) => void) {
    return <Req, R, E>(self: (req: Req) => Effect<OperationId, E, R>) =>
      (req: Req) =>

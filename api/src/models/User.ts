...
export const FirstName = S.NonEmptyString255.pipe(
  S.annotations({
    [AST.ArbitraryAnnotationId]: (): A.LazyArbitrary<string> => fc =>
      fc.string(),
  }),
  S.withDefaultMake,
)
...
export const LastName = S.NonEmptyString255.pipe(
  S.annotations({
    [AST.ArbitraryAnnotationId]: (): A.LazyArbitrary<string> =>
      fakerArb(faker => faker.person.lastName),
  }),
  S.withDefaultMake,
)
...
export class UserFromIdResolver extends Context.TagId('UserFromId')<
  UserFromIdResolver,
  { get: (userId: UserId) => Effect<User> }
>() {}
...
export const UserFromId: Schema<User, string, UserFromIdResolver> =
  S.transformOrFail(UserId, S.typeSchema(User), {
    decode: User.resolver.get,
    encode: u => Effect.succeed(u.id),
  })

export const defaultEqual = pipe(
  Equivalence.string,
...
export namespace FullName {
  export interface Encoded
    extends S.Struct.Encoded<(typeof FullName)['fields']> {}
}

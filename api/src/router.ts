...
const localOrigins = ['http://localhost:4000']

const RootRoutes = <E, R>(rpcRoutes: Layer<never, E, R>) =>
  HttpRouter.Default.use(
    Effect.fnUntraced(function* (router) {
      ...
      yield* router.get(
        ...
        MW.serverHealth(cfg.apiVersion).pipe(
          Effect.tapErrorCause(reportError('server-health error')),
        ),
      )
      yield* router.mountApp('/rpc', rpcRouter.pipe(middleware))
      yield* router.get(
        '/events',
        handleEvents.pipe(
          Effect.tapErrorCause(reportError('events error')),
          middleware,
        ),
      )
    }),
  ).pipe(
    Layer.provide([Events.Default, Router.Live.pipe(Layer.provide(rpcRoutes))]),
  )

const logServer = Effect.gen(function* () {
...
export const makeHttpServer = <E, R>(rpcRouter: Layer<never, E, R>) =>
  logServer.pipe(
    Layer.provide(
      HttpRouter.Default.unwrap(HttpServer.serve(HttpMiddleware.logger)),
    ),

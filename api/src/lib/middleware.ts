...
export const gzip = HttpMiddleware.make(app =>
  Effect.gen(function* () {
    const r = yield* app
    const body = r.body
    if (body._tag !== 'Uint8Array' || body.contentLength === 0) {
      return r
    }
    ...
    const buffer = yield* Effect.async<Buffer>(cb =>
      z.gzip(body.body, (err, r) =>
        cb(err ? Effect.die(err) : Effect.succeed(r)),
      ),
    )
...
export const cors = (options?: {
  readonly allowedOrigins?: readonly string[] | ((origin: string) => boolean)
  ...
  const headersFromRequestOptions = (
  ...
  const preResponseHandler = (
    ...
    Effect.succeed(
      HttpServerResponse.setHeaders(response, headersFromRequest(request)),
    )

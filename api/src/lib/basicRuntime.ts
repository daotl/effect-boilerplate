...
const devLog = Logger.withSpanAnnotations(Logger.logfmtLogger).pipe(
  PlatformLogger.toFile('./dev.log'),
)

const addDevLog = Logger.addScoped(devLog).pipe(
  ...
  Effect.sync(() =>
    configuredEnv && configuredEnv !== 'local-dev'
      ...
      : process.env['NO_CONSOLE_LOG']
        ? Layer.mergeAll(Logger.remove(Logger.defaultLogger), addDevLog)
        : Layer.mergeAll(
            Logger.replace(
              Logger.defaultLogger,
              Logger.withSpanAnnotations(Logger.prettyLogger()),
            ),
            addDevLog,
          ),
...
const runMainPlatform: RunMain = dual(
...
export function runMain<A, E>(
  eff: Effect<A, E, never>,
  filterReport?: (cause: Cause.Cause<E>) => boolean,
) {
  return runMainPlatform(
    eff.pipe(
      Effect.tapErrorCause(cause =>
        !filterReport || filterReport(cause)
          ? reportMainError(cause)
          : Effect.void,
      ),
...
export type RT = typeof basicRuntime.runtime extends Runtime.Runtime<infer R>
  ? R
  : never

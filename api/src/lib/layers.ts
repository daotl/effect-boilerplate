...
export const RepoDefault = StorageConfig.pipe(
  Effect.andThen(StoreMakerLayer),
  Layer.unwrapEffect,
)

export const RepoTest = StoreMakerLayer({
  url: Redacted.make('mem://'),
  prefix: 'test_',
  dbName: 'test',
})

export const EmailerLive = SendgridConfig.pipe(
  Effect.andThen(cfg => (cfg.apiKey ? Sendgrid(cfg) : FakeSendgrid)),
  Layer.unwrapEffect,
)
...
export const ApiPortTag = Context.GenericTag<{ port: number }>(
  '@services/ApiPortTag',
)

export const HttpServerLive = Effect.all([
  MergedConfig,
  Effect.serviceOption(ApiPortTag),
]).pipe(
  Effect.andThen(([cfg, portOverride]) => {
    if (portOverride.value) {
      cfg = { ...cfg, port: portOverride.value.port }
    }

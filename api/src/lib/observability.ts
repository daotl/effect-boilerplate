...
const checkTelemetryExporterRunning = Effect.promise<boolean>(() =>
  tcpPortUsed.check(4318, 'localhost'),
).pipe(
  Effect.tap(isTelemetryExporterRunning =>
    Effect.sync(() => {
      ...
      } else if (fs.existsSync('../.telemetry-exporter-running')) {
        fs.unlinkSync('../.telemetry-exporter-running')
      }
    }),
  ),
  Effect.cached,
...
const makeMetricsReader = Effect.gen(function* () {
  const isTelemetryExporterRunning =
    !isRemote && (yield* checkTelemetryExporterRunning)
...
export class MetricsReader extends Context.TagMakeId(
  'MetricsReader',
  makeMetricsReader,
)<MetricsReader>() {
...
const ConfigLive = Effect.gen(function* () {
  ...
  let props: Partial<opentelemetry.NodeSDKConfiguration> = dropUndefinedT({
    ...
    spanProcessors:
      isTelemetryExporterRunning || localConsole
        ? [
  ...
  Layer.scopedDiscard,
  Layer.provide(Layer.mergeAll(MetricsReader.Live, ResourceLive)),
)

const MetricsLive = MetricsReader.use(({ makeMetricReader }) =>
  makeMetricReader ? Metrics.layer(makeMetricReader) : Layer.empty,
).pipe(
  Layer.unwrapEffect,
